...
describe("cli/pagerank", () => {
  ...
  describe("'pagerank' command", () => {
    ...
    it("prints a message if the repo is not loaded", async () => {
      const loader = (_unused_repoId) =>
        new Promise((resolve) => resolve({status: "REPO_NOT_LOADED"}));
      ...
      expect(result).toEqual({
        ...
        stdout: [],
        stderr: [
          "fatal: repository ID foo/bar not loaded",
          "Try running `sourcecred load foo/bar` first.",
        ],
      });
    });
